...
class DrawCard {
  ...
  constructor(deck) {
    this.validParams = [
      ...
      { name: "suits", type: Array },
      { name: "ranks", type: Array },
      { name: "limit", type: Number },
      { name: "sorted", type: String },
    ];
    this.deck = deck;
    this.params = this.getValidParams();
  }

  /**
   * Get valid params from URL
   * @returns
   */
  getValidParams() {
    ...
    for (let param of queryParams) {
      ...
      const findValidParam = this.validParams.find(
        ({ name }) => name === paramName
      );
      if (findValidParam) {
        paramList[paramName] =
          findValidParam.type === Array ? paramValue.split(" ") : paramValue;
      }
    }
    return paramList;
  }
  ...
  /**
   * Validate if there are any valid param
   * @returns
   */
  hasValidParams() {
